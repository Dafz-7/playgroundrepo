...
data['Age'].fillna(data['Age'].mean(), inplace=True)  # Fill missing values in the 'Age' column with its mean
data['Years of Experience'].fillna(data['Years of Experience'].mean(), inplace=True)  # Same for 'Years of Experience'
data['Salary'].fillna(data['Salary'].mean(), inplace=True)  # Same for 'Salary'
...
x = data.iloc[:, [0, 1, 2, 3, 4]].values  # Select columns 0 to 4 as features (assuming numerical/categorical data)
...
prediksi = model.predict([[32, 1, 0, 177, 4]])
print()
print("Salary prediction:", (round(prediksi[0][0])))  # Round and print the predicted salary value